...
domains=($MAIN_DOMAIN $API_DOMAIN $FORM_DOMAIN $LANDING_DOMAIN)
rsa_key_size=4096
...
echo "### Starting nginx or reload nginx..."
docker-compose up -d nginx
# docker-compose exec nginx /bin/sh -c "envsubst '$$SSL_DOMAINS' < /etc/nginx/conf.d/default.template > /etc/nginx/conf.d/default.conf && nginx -s reload"
echo

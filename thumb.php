...
// $_GET['src'] = 'http://localhost:8000/img_data/upload-lien-he/main.jpg';
// echo $_GET['src'];
// exit;
define('VERSION', '2.8.14'); // Version of this script
//Load a config file if it exists. Otherwise, use the values below

if (file_exists(dirname(__FILE__) . '/zthumb-config.php')) {
	require_once 'zthumb-config.php';
}
...
if (!defined('FILE_CACHE_SUFFIX')) {
	define('FILE_CACHE_SUFFIX', '.timthumb.cache');
}
...
// Default water mark
if (!defined('DEFAULT_OPTION_WATERMARK')) {
	define('DEFAULT_OPTION_WATERMARK', true);
}
// Maximum image height
if (!defined('WATERMARK_OVERLAY_IMAGE')) {
	define('WATERMARK_OVERLAY_IMAGE', dirname(__FILE__) . '/' . $SITE_WATERMARKS[$site]);
}
// the watermark image
if (!defined('WATERMARK_OVERLAY_IMAGE_LOGO')) {
	define('WATERMARK_OVERLAY_IMAGE_LOGO', dirname(__FILE__) . '/' . $SITE_WATERMARK_LOGOS[$site]);
}
// the watermark image
if (!defined('NOT_FOUND_IMAGE')) {
	define('NOT_FOUND_IMAGE', dirname(__FILE__) . '/' . $DEFAULT_IMAGE);
}
// Image to serve if any 404 occurs
if (!defined('ERROR_IMAGE')) {
	define('ERROR_IMAGE', dirname(__FILE__) . '/' . $DEFAULT_IMAGE);
}
...
class timthumb {
	...
	protected function processImageAndWriteToCache($localImage) {
		...
		if ($new_width == 0 && $new_height == 0) {
			...
			// set default to original image size
			$new_width = $sData[0];
			$new_height = $sData[1];
		}

		$watermark = (bool) $this->param('wm', DEFAULT_OPTION_WATERMARK);
		...
		if ($watermark == 1) {
			...
			// calculate diff height to watermark correct y position
			$canvas_height = imagesy($canvas);
			$diff_height = 0;
			if ($canvas_height > $new_height) {
				$diff_height = ($canvas_height - $new_height) / 2;
			}

			// calculate diff height to watermark right position
			$canvas_width = imagesx($canvas);
			$diff_width = 0;
			if ($canvas_width > $new_width) {
				$diff_width = ($canvas_width - $new_width) / 2;
			}

			$overlay_gd_image = imagecreatefrompng(WATERMARK_OVERLAY_IMAGE);
			// Max watermark ratio with main image
			...
			if (max($overlay_width / $new_width, $overlay_height / $new_height) > $max_watermark_ratio) {
				...
				imagecopy($canvas, $overlay_gd_image_p, $new_width - $new_overlay_width - 20 + $diff_width, $new_height - $new_overlay_height - 20 + $diff_height, 0, 0, $new_overlay_width, $new_overlay_height);
			} else {
				// Copy the PNG to the canvas
				imagecopy($canvas, $overlay_gd_image, $new_width - $overlay_width - 20 + $diff_width, $new_height - $overlay_height - 20 + $diff_height, 0, 0, $overlay_width, $overlay_height);
			}

			// Start QRCODE watermark
			if (!empty($_GET['pid'])) {
				$pid = intval($_GET['pid']);

				$codeContents = $SITES[$site] . '/?pid=' . $pid;
			} else {
				$codeContents = $SITES[$site];
			}

			$outerFrame = 2;
			$pixelPerPoint = 5;
			include 'admin/lib/phpqrcode/qrlib.php';
			...
			// create QRCODE image
			$overlay_gd_image = imagecreate($imgW * $pixelPerPoint, $imgH * $pixelPerPoint);
			imagecopyresized(
				$overlay_gd_image,
				$base_image,
				0, 0, 0, 0,
			...
			if (max($overlay_width / $new_width, $overlay_height / $new_height) > $max_watermark_ratio) {
				...
				// Copy the PNG to the canvas
				imagecopy($canvas, $overlay_gd_image_p, $new_width - $new_overlay_width - 5 + $diff_width, $new_height - $new_overlay_height - 5 + $diff_height, 0, 0, $new_overlay_width, $new_overlay_height);
			} else {
				// Copy the PNG to the canvas
				imagecopy($canvas, $overlay_gd_image, $new_width - $overlay_width - 5 + $diff_width, $new_height - $overlay_height - 5 + $diff_height, 0, 0, $overlay_width, $overlay_height);
			}
			// End watermark QRCODE

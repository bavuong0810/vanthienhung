...
<div class="col-xs-12 sticky-left">
	<form action="index.php">
		<div class="form-group tac-vu">
			<input type="hidden" name="p" value="san-pham" />
			...
			?>
			<div class="btn-group">
				<select onchange="form_submit(this)" class="form-control">
					<option selected>Tác vụ</option>
					<option value="delete">Xóa</option>
					<option value="add_image">Thêm ảnh nhanh</option>
					<option value="list_add_image">Chọn để thêm ảnh nhanh</option>
				</select>
			</div>
			<!-- <div class="btn-group">
	...
	<div class="btn-group">
		<select id="action" onchange="search(this,'san-pham')" name="action" class="form-control">
	    ...
	    jQuery(document).ready(function($) {
	        $('input#search').keypress(function (e) {
	         var key = this.value;
	           if (e.which == 13) {
	             window.location = "index.php?p=san-pham&a=man&search=name&key="+key;
	           }
	        });        
	    }); 
	</script> -->
			<div class="btn-group">
				<select onchange="this.form.submit()" name="hienthi" class="form-control">
					...
					<option value="11" <?php if (@$_GET['hienthi'] == 11) echo 'selected'; ?>>3000</option>
					<option value="12" <?php if (@$_GET['hienthi'] == 12) echo 'selected'; ?>>5000</option>
				</select>
			</div>
			<div class="btn-group">
			...
			<a href="index.php?p=san-pham&a=man&remove_filter" class="text-danger">Xóa bộ lọc</a>
			...
			<div class="pull-right">
				<a href="index.php?p=san-pham&a=add" class="btn btn-primary">
					<i class="glyphicon glyphicon-plus"></i> Thêm mới
				</a>
				<a href="index.php?p=san-pham&a=import" class="btn btn-info">
					<i class="glyphicon glyphicon-import"></i> Nhập nhiều
				</a>
			</div>
		</div>
	</form>
...
<div class="col-xs-12">
	...
	<form id="form" method="post" action="index.php?p=san-pham&a=delete_all" role="form">
		...
		$itemFields = array(
			...
			'parent_number'	=> 'ID Nhóm',
		);
		?>

		<table class="table table-bordered table-hover table-striped table-responsive" style="width: 5000px; max-width: unset;">
			<thead>
				<tr>
					...
					<?php //foreach ($config['data']['columns']['all_shows'] as $column => $colName) {?>
					<!--<th style="width:80px"><?php //echo $colName; ?></th>-->
					<?php //}?>
			...
			<tbody>
				...
				?>
					<tr>
						...
						<td>
							...
							<a href="index.php?p=san-pham&a=delete&id=<?= $items[$i]['id'] ?>&page=<?= @$_GET['page'] ?><?php echo isset($_SESSION['product_search'])?'&search=':''?>" onClick="if(!confirm('Xác nhận xóa?')) return false;" class="text-danger" title="Xóa"><i class="glyphicon glyphicon-remove"></i></a>
						</td>
						<td style="text-align:left" class="break-word">
							<a href="index.php?p=san-pham&a=edit&id=<?= $items[$i]['id'] ?>&page=<?= @$_GET['page'] ?>"><?= $items[$i]['name_vi'] ?></a>
						...
						<td class="break-word">
							<a href="index.php?p=san-pham&a=edit&id=<?= $items[$i]['id'] ?>&page=<?= @$_GET['page'] ?>"><?= $items[$i]['name_en'] ?></a>
						</td>
						<td class="break-word">
							<a href="index.php?p=san-pham&a=edit&id=<?= $items[$i]['id'] ?>&page=<?= @$_GET['page'] ?>"><?= $items[$i]['name_ch'] ?></a>
						</td>
						<td>
							<?php if (ADMIN_IMAGE): ?>
						...
						<td>
							<input type="checkbox" onclick="on_check(this,'#_sanpham','is_hot','<?= $items[$i]['id'] ?>')" <?php if ($items[$i]['is_hot'] == 1) echo 'checked="checked"'; ?>>
						</td>
				...
				<input class="chk_box" type="checkbox" onclick="on_check(this,'#_sanpham','sp_moi','<?= $items[$i]['id'] ?>')" <?php if ($items[$i]['sp_moi'] == 1) echo 'checked="checked"'; ?>>
						...
						<td>
							<input type="checkbox" onclick="on_check(this,'#_sanpham','sp_hot','<?= $items[$i]['id'] ?>')" <?php if ($items[$i]['sp_hot'] == 1) echo 'checked="checked"'; ?>>
						</td>
						<td>
						...
						<?php //foreach ($config['data']['columns']['all_shows'] as $column => $name) {?>
						<!--
						<td>
							<input type="checkbox" onclick="on_check(this,'#_sanpham','<?php //echo $column; ?>','<?//= $items[$i]['id'] ?>')" <?php //if ($items[$i][$column] == 1) echo 'checked="checked"'; ?>>
						</td>-->
						<?php //}?>

						<td>
							<input type="checkbox" onclick="on_check(this,'#_sanpham','is_completed','<?= $items[$i]['id'] ?>')" <?php if ($items[$i]['is_completed'] == 1) echo 'checked="checked"'; ?>>
						</td>
						<td>
							<input type="checkbox" onclick="on_check(this,'#_sanpham','is_new','<?= $items[$i]['id'] ?>')" <?php if ($items[$i]['is_new'] == 1) echo 'checked="checked"'; ?>>
						</td>
						<td>
							<a href="/<?php echo $items[$i]['alias_vi']; ?>.html" class="text-success" target="_blank"><i class="glyphicon glyphicon-eye-open"></i></a>
							&nbsp;&nbsp;&nbsp;
...
?>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			cloneProduct(<?php echo $_GET['clone_id']; ?>, 'san-pham');
		});
	</script>

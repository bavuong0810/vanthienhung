<?php
if (!empty($items[0]['group_quantity']) && is_numeric($items[0]['group_quantity'])) {
    $inGroupProducts = $d->o_fet("SELECT `id`, `name_en`, `group_pos`, `group_quantity`, `part_number` from `#_sanpham` WHERE `hien_thi` = 1 AND `product_type` = " . ProductConfigs::TYPE_PRODUCT . " AND `group_quantity` = {$items[0]['group_quantity']} ORDER BY `group_pos` ASC");
}

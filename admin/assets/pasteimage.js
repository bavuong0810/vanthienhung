...
function pasteimage(target, callback) {
		...
		if(typeof(callback) == "function") {
			...
			$(target).bind("paste", (e) => doPaste(e, target));
			...
			if (!window.Clipboard) { // Firefox
				...
				$(target).prepend(pasteCatcher);
			}
		}
		// Handle paste event
		function doPaste(e, target)  { 
					...
					if (items) {
						...
						for (var i = 0; i < items.length; i++) { // removed: i < items.length, items[i].type.indexOf("image") !== -1
							if (items[i].type.indexOf("image") !== -1) {
								...
								reader.onload = (event) => {
									callback(event.target.result, target || '#' . event.target.id); //event.target.results contains the base64 code to create the image
								};

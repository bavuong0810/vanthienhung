	...
	if($_REQUEST['a'] == 'man'){
		...
		if(isset($_GET['loaitin']) && $_GET['loaitin'] <> ''){
			...
			if($_GET['loaitin'] == 0){
			    ...
			    $loaitin = $d->o_fet("select id, category_id from #_danhmuc_khachhang where hien_thi = 1");
			    $category_id = show_menu_khachhang_hd($loaitin,@addslashes($_GET['loaitin']));
			    $category_id = trim($category_id,',');
			    $category_id = @addslashes($_GET['loaitin']) .','.$category_id;
			    $items = $d->o_fet("select * from #_khachhang where FIND_IN_SET(category_id,'$category_id') <> 0 order by so_thu_tu asc");
			}
			...
			if($seach == 'id'){
				...
				$items = $d->o_fet("select * from #_khachhang where name_vi like '%$key%'");
			}
		}

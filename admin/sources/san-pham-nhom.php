...
function showdulieu()
{
	global $d, $items, $paging, $loai, $groups, $brands, $fields, $config;
	...
	if (isset($_GET['remove_filter'])) {
		...
		header('location: index.php?p=san-pham-nhom&a=man');
		exit;
	}
	...
	if ($_REQUEST['a'] == 'man') {
		...
		$wheres  = array('style = 0', 'product_type = ' . ProductConfigs::TYPE_PRODUCT_GROUP);
...
function luudulieu()
{
	...
	if ($id != '') {
		...
		$data['tai_trong_nang'] = $d->clear(addslashes($_POST['tai_trong_nang']));
		...
		if ($d->update($data)) {
			...
			// }
			///
			$d->redirect($referer ?: "index.php?p=san-pham-nhom&a=man&page=" . @$_REQUEST['page'] . "");
		} else {
			// echo mysqli_error();
			$d->transfer("Cập nhật dữ liệu bị lỗi", $referer ?: "index.php?p=san-pham-nhom&a=man");
		}
	} else {
		if (!empty($_POST['file2_clipboard'])) {
			$data['image_path'] = $_POST['file2_clipboard'];
		...
		if ($idsp = $d->insert($data)) {
			...
			// }
			///
			$d->redirect($referer ?: "index.php?p=san-pham-nhom&a=man");
		} else {
			echo mysql_error();
			$d->transfer("Thêm dữ liệu bị lỗi!", $referer ?: "index.php?p=san-pham-nhom&a=man");
		}
	}
}
...
function xoadulieu()
{
	...
	if (isset($_GET['id'])) {
		...
		if ($d->o_que("delete from #_sanpham where id='" . $id . "'")) {
			$d->redirect("index.php?p=san-pham-nhom&a=man&page=" . @$_REQUEST['page'] . "");
		} else
			$d->transfer("Xóa dữ liệu bị lỗi", "index.php?p=san-pham-nhom&a=man");
	} else $d->transfer("Không nhận được dữ liệu", "index.php?p=san-pham-nhom&a=man");
}
...
function xoadulieu_mang()
{
	...
	if (isset($_POST['chk_child'])) {
		...
		if ($d->o_que("delete from #_sanpham where id in ($chuoi)")) {
			...
			$d->redirect("index.php?p=san-pham-nhom&a=man&page=" . @$_REQUEST['page'] . "");
		} else $d->transfer("Xóa dữ liệu bị lỗi", "index.php?p=san-pham-nhom&a=man");
	} else $d->redirect("index.php?p=san-pham-nhom&a=man&page=" . @$_REQUEST['page'] . "");
}

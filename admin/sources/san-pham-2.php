...
switch ($a) {
	...
	case 'toggle_image':
		toggle_image();
		break;
	case 'show_bulk_add_image':
		show_bulk_add_image();
		$template = @$_REQUEST['p'] . "/bulk-add-image";
		break;
	default:
		$template = "index";
}
...
function showdulieu()
{
	global $d, $items, $paging, $loai, $groups, $brands, $fields, $parents, $a, $product, $config;
	...
	if (isset($_GET['remove_filter'])) {
		...
		header('location: index.php?p=san-pham&a=man');
		exit;
	}
	...
	if ($_REQUEST['a'] == 'man') {
		...
		$wheres  = array('style = 0', 'product_type = ' . ProductConfigs::TYPE_PRODUCT);
		...
		if (isset($_GET['search'])) {
			if (!empty($_POST['fields']) || !empty($_SESSION['product_search'])) {
				...
				foreach ($fields as $index => $field) {
					...
					if (in_array($field['name'], ['brand_id', 'ten_us', 'ten_ch'])) {
						if ($field['value'] == 1) {
							$wheres[] = '('.$field['name'].' IS NOT NULL AND '.$field['name'].' <> \'\')';
						} else {
							$wheres[] = '('.$field['name'].' IS NULL OR '.$field['name'].' = \'\')';
						}
		...
		if (isset($_REQUEST['id'])) {
			...
			$items[0]['name_json'] = json_decode($items[0]['name_json'], 1);
			$product = $items[0];
...
function luudulieu()
{
	...
	if ($id != '') {
		...
		$data['code_3'] = $d->clear(addslashes($_POST['code_3']));
		...
		$data['cost'] = intval($d->clear(addslashes($_POST['cost'])));
		...
		//add title for some attributes
		$attributes = array();
		$part_number_title = $_POST['part_number_title'];
		$specification_title = $_POST['specification_title'];
		$loai_title = $_POST['loai_title'];
		$bao_hanh_title = $_POST['bao_hanh_title'];
		$mfg_year_title = $_POST['mfg_year_title'];
		$group_pos_title = $_POST['group_pos_title'];
		$group_quantity_title = $_POST['group_quantity_title'];

		$brand_title = $_POST['brand_title'];
		$model_title = $_POST['model_title'];

		if( $part_number_title ){
			$attributes['part_number_title'] = $part_number_title;
		}
		if( $specification_title ){
			$attributes['specification_title'] = $specification_title;
		}
		if( $loai_title ){
			$attributes['loai_title'] = $loai_title;
		}
		if( $bao_hanh_title ){
			$attributes['bao_hanh_title'] = $bao_hanh_title;
		}
		if( $mfg_year_title ){
			$attributes['mfg_year_title'] = $mfg_year_title;
		}
		if( $group_pos_title ){
			$attributes['group_pos_title'] = $group_pos_title;
		}
		if( $group_quantity_title ){
			$attributes['group_quantity_title'] = $group_quantity_title;
		}
		if( $brand_title ){
			$attributes['brand_title'] = $brand_title;
		}
		if( $model_title ){
			$attributes['model_title'] = $model_title;
		}

		//add attributes to columns name_json
		$data['name_json'] = '';
		if( !empty($attributes) ){
			$data['name_json'] = $d->clear(addslashes(json_encode($attributes)));
		}

		//$data['name_json'] = $d->clear(addslashes(json_encode($_POST['name_json'])));
		// $extraNum = 10;
		...
		if ($d->update($data)) {
			...
			// }
			///
			$d->redirect($referer ?: "index.php?p=san-pham&a=man&page=" . @$_REQUEST['page'] . "");
		} else {
			// echo mysqli_error();
			$d->transfer("Cập nhật dữ liệu bị lỗi", $referer ?: "index.php?p=san-pham&a=man");
		}
	} else {
		if (!empty($_POST['file2_clipboard'])) {
			$data['image_path'] = $_POST['file2_clipboard'];
		...
		if ($idsp = $d->insert($data)) {
			...
			// }
			///
			$d->redirect($referer ?: "index.php?p=san-pham&a=man");
		} else {
			echo mysql_error();
			$d->transfer("Thêm dữ liệu bị lỗi!", $referer ?: "index.php?p=san-pham&a=man");
		}
	}
}
